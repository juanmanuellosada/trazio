...
describe("reportSectionError — el error real de supabase-js no es un `instanceof Error`", () => {
  it("un fallo de red real (objeto plano que arma `PostgrestBuilder` cuando `fetch` rechaza, sin `.throwOnError()`) se traduce como corte de conexión", () => {
    reportSectionError({ message: "TypeError: Failed to fetch", details: "", hint: "", code: "" });
