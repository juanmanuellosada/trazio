...
La aplicación SHALL registrar, disponibles desde cualquier pantalla, el acorde
`G` seguido de `I` (Bandeja de entrada), `T` (Hoy), `U` (Próximos), `C`
(Completado) o `A` (Hábitos); `S` para abrir el buscador; `Q` para abrir el alta
rápida de tarea; `E` para abrir el alta de un nuevo evento de calendario; y
`Ctrl/Cmd+Z` para deshacer. `G A` y `E` SHALL registrarse igual que el resto de
los atajos de este requisito, pero no navegan a ningún lado en esta fase: las
pantallas de Hábitos y de eventos de calendario son de fases posteriores.
...
- **WHEN** se presiona `S` estando en la pantalla Hoy, sin foco en un campo de
  texto
...
`G A` y el atajo general `E` de nuevo evento de calendario SHALL registrarse y
SHALL responder a la combinación de teclas sin producir un error, pero no
SHALL navegar ni abrir nada en esta fase: la pantalla de Hábitos (fase 3) y el
alta de eventos de calendario (fase 4) todavía no existen.

#### Scenario: G A no navega a ningún lado todavía

- **WHEN** se presiona `G` y luego `A` dentro de la ventana del acorde
- **THEN** la combinación se reconoce como válida y no produce ningún error,
  pero no ocurre ninguna navegación visible

#### Scenario: E como atajo general no abre nada todavía

- **WHEN** se presiona `E` sin el detalle de una tarea abierto y sin foco en
  un campo de texto
- **THEN** la combinación se reconoce como válida y no produce ningún error,
  pero no se abre ningún alta de evento
