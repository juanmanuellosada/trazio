...
- [x] 9.6 Criterio: todos los atajos funcionan y ninguno se dispara escribiendo en un campo de texto, salvo deshacer (verificado el menú contextual de tarea completo — `T`, `Y`, `V`, `⇧Ctrl+C`, `Ctrl⇧N`, `⇧Supr` — y `⇧S` en un proyecto, y la guarda de foco: Ctrl/Cmd se dispara en campos de texto, teclas sueltas no, `⇧Supr` sigue bloqueado; el bug de `Y` sin abrir el selector de prioridad ya está corregido con un `requestAnimationFrame` en `task-detail-content.tsx`, confirmado en el navegador)
