...
**Arreglado en esta tanda**: la identidad de un bloque de evento pasa a ser `calendario + evento` (`eventBlockId`, nuevo en `screen-blocks.ts`, mismo patrón que ya usaba `habitBlockId`), no el id crudo solo. `eventToCalendarBlock` arma `block.id` con esa clave compuesta y `eventsById` (`screen-calendar.tsx`) indexa por ella — como el resto de la pantalla (menú contextual, `handleSelectBlock`, `handleMoveOrResize`, la comparación de `pendingEventUpdate`) ya busca por `block.id`, no hizo falta tocar esos call sites, solo la construcción del id y el índice. Se revisó además cada otro lugar de `lib/calendar/` que buscaba un evento por su id crudo sin `calendarId`: el parche optimista de `useUpdateEvent` y la baja del caché de `useDeleteEvent` tenían el mismo agujero (una colisión de id entre calendarios podía parchear/borrar del caché el evento equivocado mientras la mutación estaba en vuelo), corregidos para exigir también `calendarId`. Lo que se le manda a Google (`event.id`/`event.calendarId` de `CalendarEventInstance`, en `applyEventUpdate`/`useEventDeleteFlow`/`EditEventDialog`) no se tocó: sigue siendo el id crudo, sin pasar por `eventBlockId` — la identidad compuesta es solo de la interfaz. Reproducido primero el defecto real (dos calendarios con eventos sembrados con el mismo `evt-1`, contra el simulador, revirtiendo momentáneamente `eventBlockId` a devolver el id crudo): clic en "Reunión Personal" abría el formulario de "Standup Trabajo", igual que reportó la verificación anterior. Con el arreglo restaurado, la misma prueba en el navegador (conectar, sembrar el mismo id en dos calendarios, habilitar los dos, editar uno y eliminar el otro por el menú contextual) pasa de punta a punta: cada acción cae sobre su evento, y el que no se tocó sigue en la grilla y en el mock. Test de regresión para la colisión de id: `lib/calendar/screen-blocks.test.ts` (`eventToCalendarBlock` arma ids distintos para eventos de calendarios distintos con el mismo id crudo) y `components/calendar/screen-calendar-event-identity.test.tsx` (montando `ScreenCalendar` de verdad, con dos eventos que colisionan en id, abrir el diálogo de edición de uno nunca muestra el otro).
...
**Arreglado en esta tanda**: la causa era exactamente la descrita arriba, no otra — `useUpdateEnabledCalendars` pasa a ser optimista (mismo patrón de `onMutate`/`onError` con revert que ya usa `useUpdateEvent`), parchando `enabledCalendarIds` en el caché de `useGoogleCalendars` al instante. Con eso, un segundo tilde inmediato ya parte de lo que dejó el primero en el caché, en vez del valor de antes de la primera tilde.
